<?php /*a:2:{s:68:"E:\PHP\www\spladmin\application\admin\view\privilege_menu\index.html";i:1545027459;s:54:"E:\PHP\www\spladmin\application\admin\view\iframe.html";i:1545366211;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
...
<script src="/public/admin/js/iframe.js"></script>
    ...
    <script type="text/javascript">
        ...
        var fieldList = [
            ...
            {
            valign:"middle",field:"title",title:"菜单名称"
            },
            {
            valign:"middle",field:"icon",title:"图标",formatter:function (value,row,index){return '<i class="'+ row.icon +'"></i>'}
            },
            {
            valign:"middle",field:"href",title:"链接"
            },
            {
            valign:"middle",field:"show_status",title:"显示",formatter:function (value,row,index){return statusFormatter('show_status',value,row,index);}
            },
            {
            valign:"middle",field:"sort",title:"排序"
            }
        ];
        var columns = [{
            align: 'center',
        ...
        function changeState(id,field,value) {
            $.ajax({
                url:'<?php echo url("PrivilegeMenu/changeState"); ?>',
                type:'post',
                dataType:'json',
        ...
        function add () {
            ...
            layer.open({
                ...
                shade:0,//隐藏遮罩层
                area : area,
                content: '<?php echo url("PrivilegeMenu/add"); ?>',
                btn:['确定','重置'],
                btnAlign:'c',
        ...
        function handleEdit(id){
            ...
            layer.open({
                ...
                shade:0,//隐藏遮罩层
                area : area,
                content: '<?php echo url("PrivilegeMenu/edit"); ?>?id=' + id,
                btn:['保存','取消'],
                btnAlign:'c',
                yes:function (index,layero) {
                    var iframe = layero.find("iframe")[0].contentWindow.document;
        ...
        function handleDelete(ids) {
            $.ajax({
                url:'<?php echo url("PrivilegeMenu/delete"); ?>',
                type:'post',
                dataType:'json',
                data:{ids:ids},
                success:function (response) {
        ...
        function initDateRange() {
            ...
            $('.dateRange').daterangepicker(
                ...
                function (start, end) {
                    $(this).val(start.format('YYYY-MM-DD HH:mm:ss') + ' - ' + end.format('YYYY-MM-DD HH:mm:ss'));
                }
            );
            //初始化显示当前时间为空
            $('.dateRange').val('');
        ...
        function oTableInit() {
            $('#table').bootstrapTable({
                url:'<?php echo url("PrivilegeMenu/index"); ?>',
                ...
                iconSize: 'sm',//按钮控件大小
                pagination:true,                //分页条
                onlyInfoPagination:true,       //仅显示总记录数
