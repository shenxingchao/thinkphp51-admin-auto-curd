<?php /*a:2:{s:67:"E:\PHP\www\spladmin\application\admin\view\privilege_role\edit.html";i:1545104288;s:54:"E:\PHP\www\spladmin\application\admin\view\iframe.html";i:1545366211;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
    ...
    <div class="container-fluid content-wrapper-box">
        <div class="content-wrapper">
            <form class="form-horizontal editForm" action="" method="post" enctype="multipart/form-data">
                    ...
                    <div class="col-xs-8">
                        <input type="text" value="<?php echo htmlentities($data['role_name']); ?>" name="role_name" class="form-control" required data-msg="角色名称不能为空">
                    </div>
                </div>
                <div class="form-group">
                    <label class="control-label col-xs-2">
...
<script src="/public/admin/js/iframe.js"></script>
    ...
    <script type="text/javascript">
        $(function () {
            ...
            //ajax获取menu配置
            $.ajax({
                url:'<?php echo url("PrivilegeRole/privilegeMenuNodes"); ?>',
                type:'get',
                dataType:'json',
                success:function (response) {
                    if(response.code === 0){
                        var privilege_menu_ids = "<?php echo htmlentities($data['privilege_menu_ids']); ?>".split(',');
            ...
            //ajax获取src配置
            $.ajax({
                url:'<?php echo url("PrivilegeRole/privilegeSrcNodes"); ?>',
                type:'get',
                dataType:'json',
                success:function (response) {
                    if(response.code === 0){
                        var privilege_src_ids = "<?php echo htmlentities($data['privilege_src_ids']); ?>".split(',');
        ...
        function fileInit() {
            ...
            $('.fileUpload').fileupload({
                ...
                add: function (e, data) {
                    ...
                    $(this).fileupload('option', 'url', "<?php echo url('PrivilegeRole/fileUpload'); ?>?data_type="+data_type);
                    data.submit();
                },
            ...
            $('body').on('click','.spl-delete-file',function () {
                ...
                var obj = $(this);
                $.ajax({
                    url:"<?php echo url('PrivilegeRole/fileDelete'); ?>",
                    type:'post',
                    dataType:'json',
